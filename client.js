...
var fs=require('fs');
var sercret='258EAFA5-E914-47DA-95CA-C5AB0DC85B11';
  ...
  o.on('data',function(e){
    if(!key){
      ...
      if(data=='<policy-file-request/>\0')
        return fs.readFile('public/ws/policy.txt',function(err,data){
          o.write(data+'\0');
        });
      //WebSocket握手
      if(key=data.match(/Sec-WebSocket-Key: (.+)|$/)[1])
        key=crypto.createHash('sha1').update(key + sercret).digest('base64'),
        o.write([
